...
//------------------------------------------------------------------------------
// audio playback buffer for similarity exploration
//------------------------------------------------------------------------------
SndBuf player => dac;
0 => player.gain;
...
while( true )
{
    ...
    // choose best match
    knnResult[0] => int best;
    windows[best].fileIndex => int fi;
    files[fi] => string fname;
    windows[best].windowTime => float wt;

    // load + seek
    me.dir() + fname => player.read;

    // seconds -> samples for SndBuf.pos
    (wt * second / samp) $ int => player.pos;

    // play longer before next query
    600::ms => now;

    // (optional) silence between grains so it feels “granular”
    0.0 => player.gain;
    20::ms => now;
    0.9 => player.gain;
